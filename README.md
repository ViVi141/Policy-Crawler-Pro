...
#### 5. 构建时网络连接失败怎么办？
系统已配置国内镜像源（清华大学镜像），但如仍遇到网络问题：

**方案一：检查 Docker Desktop**
确保 Docker Desktop 正在运行，并检查网络设置。

**方案二：配置代理（如果使用代理）**
在 Docker Desktop 设置中配置代理，或设置环境变量：
```bash
# 在 .env 文件或系统环境变量中设置
...
**方案三：手动配置镜像源**
如果自动配置失败，可以手动编辑 Dockerfile 中的镜像源地址。

**方案四：使用 VPN 或更换网络**
网络环境可能无法访问某些镜像源，建议使用 VPN 或更换网络环境。
